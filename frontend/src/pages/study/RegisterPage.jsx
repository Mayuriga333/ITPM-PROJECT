...
const subjects = [
  'Java', 'Python', 'C++', 'C#', 'DSA', 'CS',
  'OOP', 'MERN', 'Springboot', '.NET', 'Statistics',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (type === 'volunteer') {
        ...
        storage.setCurrentVolunteer(response.data?.data || response.data);
        toast.success('Registration successful!');
        navigate('/volunteer/dashboard');
      } else {
        const response = await studentAPI.register(formData);
        storage.setCurrentStudent(response.data?.data || response.data);
        toast.success('Registration successful!');
        
        if (volunteerId) {
          navigate(`/study/request/${volunteerId}`);
        } else {
          navigate('/discovery');
        }
      }
