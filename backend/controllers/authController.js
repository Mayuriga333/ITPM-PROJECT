 ...
 * also block here so the client receives a clear error on login.
 */
const login = async (req, res) => {
  try {
    ...
    // Look up linked P3 study profiles so the frontend can access the correct IDs
    let studyVolunteer = null;
    let studyStudent = null;

    if (user.role === 'Volunteer') {
      studyVolunteer = await StudyVolunteer.findOne({ $or: [{ user: user._id }, { email: user.email }] });
    } else if (user.role === 'Student') {
      studyStudent = await StudyStudent.findOne({ $or: [{ user: user._id }, { email: user.email }] });
    }
    ...
    res.json({
      ...
      token,
      user: sanitizeUser(user),
      studyVolunteer: studyVolunteer || undefined,
      studyStudent: studyStudent || undefined,
    });
  } catch (err) {
...
const getMe = async (req, res) => {
  try {
    ...
    if (req.user.role === 'Volunteer') {
      studyVolunteer = await StudyVolunteer.findOne({ $or: [{ user: req.user._id }, { email: req.user.email }] });
    } else if (req.user.role === 'Student') {
      studyStudent = await StudyStudent.findOne({ $or: [{ user: req.user._id }, { email: req.user.email }] });
    }
